...
| 2 | **P2** | **Profile Modal "Save" Button Polish** | UI / Profile Modal | 🔴 Todo | Simplify button text from "Save & Switch to Dancer" to "Save" and reduce its oversized dimensions. |
| 3 | **P2** | **Remove Greeting Header on Discover Tab** | UI / Discover Tab | 🔴 Todo | Remove the bulky greeting banner ("Good evening Mayank, what's your rhythm today") to recover vertical screen real estate. |
| 4 | **P1** | **Fix Featured Spotlight Carousel & Exclude Booked Classes** | Discover / Events | 🔴 Todo | Fix spotlight carousel image loading / cycling, and prevent classes the user has already booked from showing in the featured spotlight. |
| 5 | **P1** | **Home Top Banner: Upcoming Booking & QR Code Trigger** | Discover / Bookings | 🔴 Todo | Update the top hero banner on Discover to feature the user's next upcoming booking instead of a generic class, opening the ticket QR code directly on click. |
| 6 | **P1** | **Sync "My Bookings" Count Indicator with Bookings Tab List** | State Management / Bookings | 🔴 Todo | Resolve data inconsistency where badge/popup shows 4 booked classes but the Bookings tab only renders 3. |
| 7 | **P2** | **Add "Mark as Unread" Option in Notifications** | Notifications | 🔴 Todo | Enable users to toggle a notification's status back to unread. |
...
### 2. Profile Modal "Save" Button Polish
* **Status**: 🔴 Todo
* **Priority**: **P2**
* **Impacted Components**: `src/components/modals/ProfileModal.tsx`, `src/styles/modal.css` (or relevant profile styles)
* **Problem**: The CTA button labeled "Save & Switch to Dancer" is oversized and contains overly verbose/vague copy.
* **Solution**: Update the button label to simply "Save" and adjust padding/font sizes to match standard modal primary button guidelines.

### 3. Remove Greeting Header on Discover Tab
* **Status**: 🔴 Todo
* **Priority**: **P2**
...
### 4. Fix Featured Spotlight Carousel & Exclude Booked Classes
* **Status**: 🔴 Todo
* **Priority**: **P1**
...
### 5. Home Top Banner: Upcoming Booking & QR Code Trigger
* **Status**: 🔴 Todo
* **Priority**: **P1**
...
### 6. Sync "My Bookings" Count Indicator with Bookings Tab List
* **Status**: 🔴 Todo
* **Priority**: **P1**
...
### 7. Add "Mark as Unread" Option in Notifications
* **Status**: 🔴 Todo
* **Priority**: **P2**
